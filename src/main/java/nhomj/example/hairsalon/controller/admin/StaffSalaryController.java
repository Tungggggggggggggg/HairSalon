...
public class StaffSalaryController {
    ...
    public String saveSalary(@ModelAttribute("newSalary") StaffSalary staffSalary, Model model) {
        try {
            staffSalaryService.saveStaffSalary(staffSalary);
        } catch (Exception e) {
            model.addAttribute("errorMessage", "Error saving salary: " + e.getMessage());
            return "admin/dashboard/salary_management"; // Return to the same page with error
        }
